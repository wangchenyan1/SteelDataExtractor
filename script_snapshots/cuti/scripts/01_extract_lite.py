...
## 规则
1. **样品识别**：同一合金不同固溶/时效/冷轧 = 1 个 sample + 多个 conds（禁止按热处理态拆 sample）
2. **condition 筛选**：只建有明确性能数字的 cond；时效矩阵禁止全表展开；优先 as-ST / peak / over-aged / 最优与对比点；全文 conds ≤15
3. **conds[].name**：极简代号（ST900/A450-120min），不要堆完整工艺
4. **conds[].proc**：每个 cond 独有，不能笼统
5. **conds[].type=post_test**：断后样品 → 不进库
6. **figs[].ms**：true=组织图；false=断口/曲线/示意图/XRD
7. **figs[].post**：true=断后图
8. **figs[].fid / pi**：图号与 [FIGURE_PLACEHOLDER_n] 对应；多 panel 拆开
9. **sid/cid**：须对应 samples/conds；无法判断填 null
10. 省略 caption/组织描述；性能无则 ""，不要 null
"""
