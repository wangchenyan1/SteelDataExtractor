"""Cu-Ti 自动 fallback 抽取脚本。

策略：
1. 尝试标准版 01_extract.py（完整 schema，含 condition ≤15 筛选）
2. 标准版因 JSON 截断失败 → 切瘦身版 01_extract_lite.py（同样含 condition 筛选）
3. 瘦身版也失败 → 记录到 failed_papers.txt
"""
