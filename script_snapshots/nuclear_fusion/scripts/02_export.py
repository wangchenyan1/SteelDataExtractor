"""核聚变低磁奥氏体不锈钢焊接文献数据库导出脚本。

从 nuclear_fusion/outputs/<paper_id>/paper.json 导出 final_database.csv / xlsx。

设计原则：
- 一行 = 一个 sample × 一个 condition 组合
- 图片由 LLM 输出的 figure → sample/condition 映射决定
- 只有 is_microstructure_image=true 且 is_post_test_image=false 的组织图进入对应列
- 基础导入列名与 non_magnetic / cuti 导出保持一致；核聚变特有字段仅作为增量列补充
"""
...
METHOD_COLUMNS = {
    ...
    "EBSD": ("EBSD照片编号", "EBSD照片链接", "EBSD照片", "EBSD比例尺", "EBSD照片描述"),
}
...
MECH_FIELDS = [
    ...
    ("fracture_toughness", "FractureToughness_name", "FractureToughness_value", "FractureToughness_unit", "Fracture toughness"),
]
MAG_FIELDS = [
...
PROCESS_TEXT_FIELDS = [
    ...
    ("heat_treatment_condition", "HeatTreatmentCondition"),
]
PROCESS_METRIC_FIELDS = [
    ("test_temperature", "TestTemperature_name", "TestTemperature_value", "TestTemperature_unit", "Test temperature"),
]
...
EXPORT_HEADERS = [
    ...
    "TestTemperature_name",
    "TestTemperature_value",
    "TestTemperature_unit",
    "Yield_name",
    "Yield_value",
    "Yield_unit",
    "Tensile_name",
    ...
    "Elongation_value",
    "Elongation_unit",
    "FractureToughness_name",
    "FractureToughness_value",
    "FractureToughness_unit",
    "Permeability_name",
    "Permeability_value",
    "Permeability_unit",
    "GrainSize_name",
...
for _method in METHODS:
    EXPORT_HEADERS.extend(METHOD_COLUMNS[_method])
...
def build_rows_for_paper(paper_json_path: Path) -> tuple[list[dict[str, str]], dict]:
    ...
    for sample in samples:
        ...
        for cond in sample_conditions:
            ...
            if cond:
                row["ComponentType"] = str(cond.get("component_type") or "")
                row["JointOrMaterialType"] = str(cond.get("joint_or_material_type") or "")
                for json_key, excel_col in PROCESS_TEXT_FIELDS:
                    val = cond.get(json_key)
                    if val not in (None, ""):
                        row[excel_col] = str(val)
                for json_key, name_col, value_col, unit_col, default_name in PROCESS_METRIC_FIELDS:
                    _set_metric(row, name_col, value_col, unit_col, cond.get(json_key), default_name)
            ...
            if cond:
                component_type = cond.get("component_type") or ""
                joint_type = cond.get("joint_or_material_type") or ""
                if component_type:
                    text_parts.append(f"component_type={component_type}")
                if joint_type:
                    text_parts.append(f"joint_or_material_type={joint_type}")
                for key, label in (
                    ("welding_process", "welding_process"),
                    ("filler_material", "filler_material"),
                    ("heat_treatment_condition", "heat_treatment_condition"),
                ):
                    val = cond.get(key) or ""
                    if val:
                        text_parts.append(f"{label}={val}")
                temp = cond.get("test_temperature")
                if isinstance(temp, dict):
                    temp_value = temp.get("value") or ""
                    temp_unit = temp.get("unit") or ""
                    if temp_value or temp_unit:
                        text_parts.append(f"test_temperature={temp_value} {temp_unit}".strip())
            if cond_proc:
...
def write_xlsx(path: Path, headers: list[str], rows: list[dict[str, str]], root: Path) -> int:
    ...
    image_cols = {METHOD_COLUMNS[method][2] for method in METHODS}
    ...
    for col_idx, header in enumerate(headers, start=1):
        ...
        elif "描述" in header or "description" in header or header == "Text":
            ws.column_dimensions[letter].width = 45
        elif "链接" in header or "links" in header:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="核聚变低磁奥氏体不锈钢焊接文献数据库导出")
    ...
    print(f"[INFO] 导出列数: {len(headers)}")

    if args.paper_id:
        paper_ids = args.paper_id
    elif not OUTPUT_DIR.exists():
        print(f"[WARN] 抽取输出目录不存在，导出空表: {OUTPUT_DIR}")
        paper_ids = []
    else:
        paper_ids = sorted(
