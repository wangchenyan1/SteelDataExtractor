...
| 视图 | 职责 |
| --- | --- |
| 项目 | 选择 / 新建项目（模板 + 文档类型 + 名称） |
| 配置 | 勾选字段、编辑性能阶段、白话配置过滤策略 |
| 文献与运行 | 加 PDF / 勾选文献 / 整篇或批量抽取；顶栏看任务进度与取消 |
| 复核 | 左原文 · 右层级结果 · 规则未通过异色展示；可对照不同 run、做单篇分析 |
...
PDF（可选，需 UNIPARSER_API_KEY）
  → 项目本地 paper.md + 图片
  → 按文档类型剪裁（论文去掉参考文献 / 致谢等；专利去掉摘要、权利要求等）
  → 骨架 entity（样品 / 状态，不传图、不抽 figures）
  → 各性能阶段（按项目配置的 steps；传入骨架 + 剪裁后正文）
  → 图片 figure_extract（图多时自动分批）
  → 规则校验 → 合并导出（paper.json，字段带 excerpt + location + status）
```

性能阶段可在配置页自定义（如「骨架 → 力学性能 → 电导性能」）。
规则未通过的性能值与图片**保留在结果树中**，标记为 `status=rejected_by_rule` 并显示原因；导出时可选择是否包含。

已有 `paper.md` 的文献默认不再调用 UniParser。超长文档的自动 chunk 尚未做，目前仍是剪裁后整份进入各阶段。

失败时文献表和结果来源会标 **失败·骨架已完成** 或 **失败·骨架未完成**。同篇、同模式、同模型再抽时，若骨架已落盘会跳过 entity 续跑；性能步与抽图步失败会自动重试（最多 5 次），骨架步不自动重试。

## 启动工作台

抽取走真实多模态后端。在仓库根目录放 `.env`，至少配置 `LLM_API_KEY` 或 `GPUGEEK_API_KEY`；解析 PDF 另需 `UNIPARSER_API_KEY`。
...
网页里用模型下拉选择本次抽取模型（各任务各自带模型参数，互不覆盖）。`.env` 里的 `LLM_MODEL` / `LLM_BASE_URL` 只作为未选模型时的默认值。
...
| 勾选后批量抽 | 文献表「抽取所选」/ `POST /api/run_batch` | 服务端排队，默认 2 路并发；顶栏任务表可取消单条或全部进行中任务 |
...
取消在当前这一次模型请求结束后生效，不会从半截 HTTP 里掐断。
...
steel_extract_tool_workspace/
  ...
  tools/
    ...
    pipeline.py            # 配置驱动抽取
    batch_extract.py       # 批量排队与并发
    job_tracker.py         # 任务进度与取消
    llm_backends.py        # 多模态调用
  ...
  configs/
    ...
    projects/              # 项目覆盖层
    fields/                # 旧项目兼容配置
  example_data/            # 示例已解析文献（paper.md + 图片）
  test_runs/               # 试跑结果
  docs/
```
...
3. 抽取：项目 `backend: "claude"`；密钥放仓库根目录 `.env`（`LLM_API_KEY` 或 `GPUGEEK_API_KEY`），模型在工作台选择。
